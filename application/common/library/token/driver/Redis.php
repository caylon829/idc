...
class Redis extends Driver
{
    ...
    public function push($k,$value){
        $value  =  (is_object($value) || is_array($value)) ? \GuzzleHttp\json_encode($value) : $value;
    ...
    public function pop($name,$sendtimeout=0){
        ...
        if($this->handler->lSize($this->options['prefix'].$name)>0){
            $value = $this->handler->blPop($keys,$timeout);
        }else{
            return null;
        }

        $jsonData  = \GuzzleHttp\json_encode( $value[1], true );
